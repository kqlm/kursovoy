...
screen gallery_screen():
    ...
    frame:
        has vbox
        text "Галерея" size 40

        # Перебор всех изображений в галерее
        for image, description in gallery:
            # Создание кнопки для каждого изображения
            textbutton description action Show("image_view", image=image)

        textbutton "Назад" action Return()
        
screen image_view(image):
    tag menu
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("Галерея") action Show("gallery_screen")
